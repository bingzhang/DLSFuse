...
public abstract class DLSStream{
	...
	public static final int ONE_PIPE_CAPACITY = FuseClient.ONE_PIPE_CAPACITY;
	...
	protected FuseClient createClient(){
	    return new FuseClient(this);
	}
	...
	final public void initStreamPool(final DLSListingTask listingtask,
		...
		if(persistent_multi_Streams){
			...
			final String protocol2 = "httpclient";
			/*
	...
	abstract protected Ad listCMD(DLSListingTask listingtask, String assignedThreadName, String listingFullPath) throws Exception ;
	
	
	protected Ad retrieveContents(DLSListingTask listingtask, 
		...
		Ad ad = listCMD(listingtask, assignedThreadName, path);
		if(null == ad && CHANNEL_STATE.CC_BENIGN == listingtask.getClient().channelstate){
			ad = new Ad();
		}
		if(PRINT_TIME){
