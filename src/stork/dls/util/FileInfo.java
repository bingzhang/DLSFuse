...
public class FileInfo {
     ...
     * @exception Exception if unable to parse the reply
     */
    ...
    public void parseUnixListReply(String reply) 
        ...
        if (numTokens < 8) {
            throw new Exception("Invalid number of tokens in the list reply [" + 
                                   reply + "]");
        }
        ...
        try {
            ...
            throw new Exception("Could not parse access permission bits");
        }
        ...
        try {
            ...
            try {
                ...
                throw new Exception("Invalid size number in the ftp reply [" + 
                                       previousToken + ", " + token + "]");
            }
        }
        ...
        if (ps == -1) {
            ...
            throw new Exception("Could not find date token");
        } else {
            ps = reply.indexOf(this.time, ps+month.length());
            if (ps == -1) {
                // this should never happen
                throw new Exception("Could not find time token");
            } else {
